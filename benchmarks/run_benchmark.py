...
from benchmarks.param_spaces import SVMSpace
    ...
    for model in [SVMSpace]:
        ...
        for task_id in config.TASKS:
            ...
            for train_index, test_index in kf.split(X):
                ...
                smac_train, smac_test = config.get_smac_trajectories(smac, config.NUM_ITER, X_train,
                                                                     y_train, X_test, y_test,
                                                                     seeds=config.SEEDS)

                hb_train, hb_test = config.get_smac_trajectories(hyperboost, config.NUM_ITER, X_train,
                                                                 y_train, X_test, y_test,
                                                                 seeds=config.SEEDS)
                smac_res = {
