...
def create_smac_runner(model, X, y, cv):
    ...
    def execute_from_cfg(cfg, seed=None):
        ...
        acc = cross_val_score(mdl, X, y, cv=ss)
...
def get_smac_trajectories(smac, num_iterations, X_train, y_train, X_test, y_test, seeds):
    ...
    for entry in smac.trajectory:
        ...
        loss, std = validate_model(config, X_train, y_train, X_test, y_test, seeds)
        train_trajectory[index:] = train_perf
        test_trajectory[index:] = loss
    return train_trajectory, test_trajectory


def validate_model(best_config, X_train, y_train, X_test, y_test, seeds):
    accuracies = []
    for seed in seeds:
        model = from_cfg(best_config, seed)
