...
# Changes:
# - All Local Search neighbors are checked in one batch.
...
class LocalSearch(AcquisitionFunctionMaximizer):
    ...
    def _one_iter(
        ...
        while True:
            ...
            # Check all neighbors in one batch
            s_time = time.time()
            all_neighbors = [i for i in all_neighbors]
            n_neighbors = len(all_neighbors)
            acq_vals = self.acquisition_function(all_neighbors, **kwargs)
            neighbors_looked_at += n_neighbors
            e_time = (time.time() - s_time) / n_neighbors
            for _ in range(n_neighbors):
                time_n.append(e_time)

            # Check all acquisition values
            best = np.argmax(acq_vals)
            if acq_vals[best] > acq_val_incumbent:
                self.logger.debug("Switch to one of the neighbors")
                incumbent = all_neighbors[best]
                acq_val_incumbent = acq_vals[best]
                changed_inc = True

            # Find neighbors with equal value
            neighbors = []
            for index, acq_val in enumerate(acq_vals):
